...
export const BlogSlider: React.FC<BlogSliderProps> = ({ articles }) => {
  ...
  // Duplicate for infinite scroll effect if needed, though simple scroll is fine too.
  // VisaSlider duplicates 3 times. Let's duplicate 2 times for smoother loop if we implement loop logic.
  // But VisaSlider logic:
  // if (scrollContainer.scrollLeft >= oneSetWidth) scrollContainer.scrollLeft -= oneSetWidth;
  // So we need at least 2 sets.
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (!isDragging && !isPaused) {
        scrollContainer.scrollLeft += 0.8; // Slower speed for reading
